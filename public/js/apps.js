 ...
 * @param            
 */
  $(document).on('click', '.addNewRoles', function(){
  ...
  $(document).on('click', '.addNewUser', function(){  
   ...
   $('.headerTitle').html("Create Client");
   $('.popup-content').html(data);
           showMod();
...
//fetch active projects
 function fetch_activeProject_list()
 {
   $.ajax({
    url:uri + 'project/activeProject/',
       success:function(data)
      { 
        $('#activeProjectTable tbody').html(data);
           
     }
    }); 
   
 }
 function fetch_pendingProject_list()
 {
   $.ajax({
    url:uri + 'project/pendingProject/',
       success:function(data)
      { 
 ...
 * @return {[type]}                           
 */
  $(document).on('submit','.storeRole', function(evt){
...
$(document).on('submit','.storeProject', function(evt)
{
 
  evt.preventDefault();
  var formdata = $(this).serialize();   
           $.ajax({
            url:uri + 'project/store',
            type: "POST", 
            dataType: "json",   
            data: new FormData(this),
            contentType: false,
...
/*?????????????????????????????????????PAGINATION????????????????????????????????????????????????*/
  ...
  $(document).on('click','.pendingListPagin', function(){
       var page = $(this).attr('id'); 
        $.ajax({
      url:uri + 'project/pendingProject/page/'+page,
    type: 'POST',
 ...
 var id = $(this).attr("id"); 
   ...
   $.ajax({
    url: uri+'User/edit/'+id, 
       success:function(data)
      { 
   $('.headerTitle').html("Modify Client");
   $('.popup-content').html(data);
           showMod();
